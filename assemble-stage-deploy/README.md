...
This project's goal is to demonstrate how to assemble, stage, and manage/scale applications once deployed:
- Create an uber JAR that is runnable anywhere
- Bake a Docker container image that can be run locally with no need for Java
- Push our Docker container to Docker Hub where it can be consumed by our clients
- Run multiple instances of our uber JAR pointed at a persistent database, different than the one it came bundled with.
...
We can now run three instances using Spring Boot’s profile support.
...
```
- Here, we can see that instance1 is now running on port 9000.
...
```
The JDBC properties can be summarized as follows:
...
The JPA properties can be described as follows:
